...
impl StringServerKey {
    ...
    pub fn trim_start_no_init_padding(&self, s: &FheString) -> FheString {
        ...
        for c in s.content.iter() {
            self.integer_key.bitand_assign_parallelized(
                ...
                &self
                    .integer_key
                    .scalar_eq_parallelized(&c.0, ASCII_WHITE_SPACE),
            );
...
mod tests {
    ...
    #[test]
    fn test_trim_start_no_init_padding() {
        let encrypted_str = encrypt_str(&KEYS.0, " a").unwrap();
        let trimed_encrypted_str = KEYS.1.trim_start_no_init_padding(&encrypted_str);
